...
from scripts.fat2019.spectrogram_images import (
    crop_or_pad_frames,
...
class SpectrogramImageTests(unittest.TestCase):
    ...
    def test_normalize_logmel_image_with_stats_uses_mel_band_statistics(self) -> None:
        image = np.array([[3.0, 5.0], [10.0, 14.0]], dtype=np.float32)
        mean = np.array([1.0, 2.0], dtype=np.float32)
        std = np.array([2.0, 4.0], dtype=np.float32)

        normalized = normalize_logmel_image_with_stats(image, mean=mean, std=std)
    ...
    def test_logmel_image_cache_path_supports_tag(self) -> None:
        self.assertEqual(
            str(
            ...
            "data/curated_logmel_image_m128_f512_globalmel.npz",
        )
