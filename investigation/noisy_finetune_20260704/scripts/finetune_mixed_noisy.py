...
from scripts.fat2019.labels import dataframe_to_multihot  # noqa: E402
from scripts.fat2019.metrics import calculate_overall_lwlrap  # noqa: E402
from scripts.fat2019.neural_helpers import (  # noqa: E402
    compute_pos_weight,
...
from scripts.train_logmel_cnn import (  # noqa: E402
    LogmelDataset,
    SmallLogmelCnn,
    build_optimizer,
    build_scheduler,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--noisy-loss-weight", type=float, default=NOISY_LOSS_WEIGHT)
    parser.add_argument("--curated-loss-weight", type=float, default=CURATED_LOSS_WEIGHT)
    parser.add_argument("--gaussian-noise-std", type=float, default=GAUSSIAN_NOISE_STD)
...
def run_branch(branch: BranchConfig, args: argparse.Namespace) -> BranchRunResult:
    ...
    curated_x, curated_fnames = _load_cache(branch, split="curated")
    noisy_x, noisy_fnames = _load_cache(branch, split="noisy")
    ...
    source_checkpoint_path = branch.copied_checkpoint if branch.copied_checkpoint.exists() else branch.source_checkpoint
    source_metadata_path = branch.copied_metadata if branch.copied_metadata.exists() else branch.source_metadata
    ...
    model_dir = branch.run_dir / "models"
    experiments_dir = branch.run_dir / "experiments"
    submissions_dir = branch.run_dir / "submissions"
    ...
    test_x, test_fnames = _load_cache(branch, split="test")
