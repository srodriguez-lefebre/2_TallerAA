...
def main() -> None:
    ...
    train_cache = np.load(
        logmel_image_cache_path(args.data_dir, split="curated", n_mels=args.n_mels, frames=args.frames),
        allow_pickle=False,
    )
    test_cache = np.load(
        logmel_image_cache_path(args.data_dir, split="test", n_mels=args.n_mels, frames=args.frames),
        allow_pickle=False,
    )
    x = train_cache["x"]
    x_test = test_cache["x"]
